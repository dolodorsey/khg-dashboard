...
:root{--bg:#FAFAF8;--sf:#fff;--sf2:#F5F3EF;--bd:#E8E4DD;--tx:#1A1A18;--tx2:#6B6560;--tx3:#A39E96;--ac:#E8652B;--acL:#E8652B10;--gn:#4A8F6D;--rd:#C45B4A;--yl:#D4A843;--bl:#5B7FA6;--hd:'Playfair Display:wght@400;500;600;700',Georgia,serif;--bd-f:'Outfit',system-ui,sans-serif;--mn:'JetBrains Mono',monospace;--r:8px}
...
.bg-ac{background:#E8652B15;color:var(--ac)}.bg-gn{background:rgba(74,143,109,.1);color:var(--gn)}.bg-rd{background:rgba(196,91,74,.1);color:var(--rd)}.bg-bl{background:rgba(91,127,166,.1);color:var(--bl)}.bg-mt{background:var(--sf2);color:var(--tx3)}.bg-yl{background:rgba(212,168,67,.1);color:var(--yl)}
...
function TaskBoard({ d, reload }) {
  ...
  const create=async()=>{if(!nt.title.trim())return;setBusy("new");const ex=await q("khg_master_tasks","select=task_key&order=task_key.desc&limit=1");const num=ex[0]?.task_key?parseInt(ex[0].task_key.split("-").pop())+1:95;await qi("khg_master_tasks",{task_key:`TASK-2026-${String(num).padStart(4,"0")}`,title:nt.title,category:nt.category,priority:nt.priority,assigned_to:nt.assigned_to,description:nt.description||null,status:"open",brand:"huglife",created_by:"dr_dorsey"});setNt({title:"",priority:"medium",assigned_to:"claude",category:"other",description:""});setCreating(false);await reload();setBusy(null)};
...
function SocialMedia({ d, reload }) {
  ...
  const createPost=async()=>{if(!np.caption.trim())return;setBusy("new");await qi("ghl_social_posting_queue",{brand_key:"huglife",caption:np.caption,platform:np.platform,content_type:np.content_type,status:"queued"});setNp({caption:"",platform:"instagram",content_type:"post"});setComposing(false);await reload();setBusy(null)};
  return (<div className="up">
...
export default function Dashboard() {
  ...
  const load=useCallback(async()=>{
    const [tasks,outreach,social,events,content,emails,ambassadors]=await Promise.all([
      q("khg_master_tasks","select=*&order=priority,created_at.desc&limit=100"),
      q("contact_action_queue","select=*&brand_key=in.(huglife,noir,taste_of_art,paparazzi,gangsta_gospel,sundays_best,wrst_bhvr,remix,pawchella,secret_society,beauty_beast,black_ball,snow_ball,monsters_ball,kulture,soul_sessions,underground_king,crvngs,cinco_de_mayo,block_party,parking_lot_pimpin)&order=created_at.desc&limit=100"),
      q("ghl_social_posting_queue","select=*&brand_key=in.(huglife,noir,taste_of_art,paparazzi,gangsta_gospel,sundays_best,wrst_bhvr,remix,pawchella,secret_society,beauty_beast,black_ball,snow_ball,monsters_ball,kulture,soul_sessions,underground_king,crvngs,cinco_de_mayo,block_party,parking_lot_pimpin)&order=created_at.desc&limit=50"),
      q("eventbrite_events","select=*&brand_key=in.(huglife,noir,taste_of_art,paparazzi,gangsta_gospel,sundays_best,wrst_bhvr,remix,pawchella,secret_society,beauty_beast,black_ball,snow_ball,monsters_ball,kulture,soul_sessions,underground_king,crvngs,cinco_de_mayo,block_party,parking_lot_pimpin)&event_date=gte."+new Date().toISOString().split("T")[0]+"&order=event_date.asc&limit=30"),
      q("weekly_content_schedule","select=*&brand_key=in.(huglife,noir,taste_of_art,paparazzi,gangsta_gospel,sundays_best,wrst_bhvr,remix,pawchella,secret_society,beauty_beast,black_ball,snow_ball,monsters_ball,kulture,soul_sessions,underground_king,crvngs,cinco_de_mayo,block_party,parking_lot_pimpin)&is_active=eq.true&order=day_of_week,post_time"),
      q("email_approval_queue","select=*&order=created_at.desc&limit=30"),
      q("khg_ambassadors","select=*&order=created_at.desc&limit=50"),
    ]);
    setD({tasks:tasks||[],outreach:outreach||[],social:social||[],events:events||[],content:content||[],emails:emails||[],ambassadors:ambassadors||[],});
    setLoading(false);
  },[]);
  ...
  const nav=[
    ...
    {id:"ambassadors",label:"Ambassadors",sec:"Data",ct:(d.ambassadors||[]).length},
  ];
  const secs=[...new Set(nav.map(n=>n.sec))];const titles={};nav.forEach(n=>titles[n.id]=n.label);

  const render=()=>{
    if(loading) return <div style={{textAlign:"center",padding:60,color:"var(--tx3)",fontFamily:"var(--hd)",fontSize:16}}>Loading HugLife + Events...</div>;
    switch(screen){
      ...
      case "ambassadors": return <ExtraTable name="Ambassadors" rows={d.ambassadors||[]} />;
      default: return <Overview d={d} go={go}/>;
    }
  };

  return (<><style>{css}</style><div className="app">
    <div className="sb"><div className="sb-hd"><h2>HugLife + Events</h2><p>21 Event Brands</p></div>
    ...
    <div className="mn"><div className="tb"><h1>{titles[screen]||"HugLife + Events"}</h1><span className="dt">{new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</span></div><div className="ct-a">{render()}</div></div>
  </div></>);
}
