...
const css = `
@import url('https://fonts.googleapis.com/css2?family=Outfit:wght@400;500;600;700&family=Outfit:wght@300;400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap');
*{margin:0;padding:0;box-sizing:border-box}
:root{--bg:#FAFAF8;--sf:#fff;--sf2:#F5F3EF;--bd:#E8E4DD;--tx:#1A1A18;--tx2:#6B6560;--tx3:#A39E96;--ac:#6B3FA0;--acL:#6B3FA010;--gn:#4A8F6D;--rd:#C45B4A;--yl:#D4A843;--bl:#5B7FA6;--hd:'Outfit:wght@400;500;600;700',Georgia,serif;--bd-f:'Outfit',system-ui,sans-serif;--mn:'JetBrains Mono',monospace;--r:8px}
...
.bg-ac{background:#6B3FA015;color:var(--ac)}.bg-gn{background:rgba(74,143,109,.1);color:var(--gn)}.bg-rd{background:rgba(196,91,74,.1);color:var(--rd)}.bg-bl{background:rgba(91,127,166,.1);color:var(--bl)}.bg-mt{background:var(--sf2);color:var(--tx3)}.bg-yl{background:rgba(212,168,67,.1);color:var(--yl)}
...
function TaskBoard({ d, reload }) {
  ...
  const create=async()=>{if(!nt.title.trim())return;setBusy("new");const ex=await q("khg_master_tasks","select=task_key&order=task_key.desc&limit=1");const num=ex[0]?.task_key?parseInt(ex[0].task_key.split("-").pop())+1:95;await qi("khg_master_tasks",{task_key:`TASK-2026-${String(num).padStart(4,"0")}`,title:nt.title,category:nt.category,priority:nt.priority,assigned_to:nt.assigned_to,description:nt.description||null,status:"open",brand:"apps",created_by:"dr_dorsey"});setNt({title:"",priority:"medium",assigned_to:"claude",category:"other",description:""});setCreating(false);await reload();setBusy(null)};
...
function SocialMedia({ d, reload }) {
  ...
  const createPost=async()=>{if(!np.caption.trim())return;setBusy("new");await qi("ghl_social_posting_queue",{brand_key:"good_times",caption:np.caption,platform:np.platform,content_type:np.content_type,status:"queued"});setNp({caption:"",platform:"instagram",content_type:"post"});setComposing(false);await reload();setBusy(null)};
  return (<div className="up">
...
export default function Dashboard() {
  ...
  const load=useCallback(async()=>{
    const [tasks,outreach,social,events,content,emails,gt_venues,gt_events]=await Promise.all([
      q("khg_master_tasks","select=*&order=priority,created_at.desc&limit=100"),
      q("contact_action_queue","select=*&brand_key=in.(good_times,sos)&order=created_at.desc&limit=100"),
      q("ghl_social_posting_queue","select=*&brand_key=in.(good_times,sos)&order=created_at.desc&limit=50"),
      q("eventbrite_events","select=*&brand_key=in.(good_times,sos)&event_date=gte."+new Date().toISOString().split("T")[0]+"&order=event_date.asc&limit=30"),
      q("weekly_content_schedule","select=*&brand_key=in.(good_times,sos)&is_active=eq.true&order=day_of_week,post_time"),
      q("email_approval_queue","select=*&order=created_at.desc&limit=30"),
      q("gt_venues","select=id,venue_name,city,subcategory,rating,google_maps_url&order=venue_name&limit=100"),
      q("gt_daily_events","select=*&order=event_date.asc&limit=50"),
    ]);
    setD({tasks:tasks||[],outreach:outreach||[],social:social||[],events:events||[],content:content||[],emails:emails||[],gt_venues:gt_venues||[],gt_events:gt_events||[],});
    setLoading(false);
  },[]);
  ...
  const nav=[
    ...
    {id:"gt_venues",label:"GT Venues",sec:"Data",ct:(d.gt_venues||[]).length},{id:"gt_events",label:"GT Events",sec:"Data",ct:(d.gt_events||[]).length},
  ];
  const secs=[...new Set(nav.map(n=>n.sec))];const titles={};nav.forEach(n=>titles[n.id]=n.label);

  const render=()=>{
    if(loading) return <div style={{textAlign:"center",padding:60,color:"var(--tx3)",fontFamily:"var(--hd)",fontSize:16}}>Loading Apps...</div>;
    switch(screen){
      ...
      case "gt_venues": return <ExtraTable name="GT Venues" rows={d.gt_venues||[]} />;
      case "gt_events": return <ExtraTable name="GT Events" rows={d.gt_events||[]} />;
      default: return <Overview d={d} go={go}/>;
    }
  };

  return (<><style>{css}</style><div className="app">
    <div className="sb"><div className="sb-hd"><h2>Apps</h2><p>Good Times · SOS · Help 911</p></div>
    ...
    <div className="mn"><div className="tb"><h1>{titles[screen]||"Apps"}</h1><span className="dt">{new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</span></div><div className="ct-a">{render()}</div></div>
  </div></>);
}
